...
async function checkTweets() {
    ...
    const timeline = await appOnlyClient.v2.userTimeline(user.data.id, { max_results: 5,"tweet.fields":["referenced_tweets","author_id","created_at"] })
    const mostRecentTweet = await (await timeline.fetchNext()).data.data.filter(el=>!el.referenced_tweets)[0]
